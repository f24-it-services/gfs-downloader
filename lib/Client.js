...
var Client = function () {
  function Client() {
    var baseUrl = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : GFS_BASE_URL;
  ...
  _createClass(Client, [{
    ...
    key: 'downloadField',
    value: function downloadField(generatedDate, forecastOffset, field, localPath) {
      var _this3 = this;

      var progressCb = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : NOOP;
      ...
      return this.getGribIndex(url).then(function (index) {
        ...
        return _this3.__checkFile(localPath, entry).then(function (skip) {
          if (skip) {
          ...
          return _this3.__download(entry.url, localPath, entry.start, entry.end, progressCb);
        }).then(function (path) {
    ...
    key: '__checkFile',
    value: function __checkFile(localPath, entry) {
      return new Promise(function (resolve, reject) {
        _fs2.default.stat(localPath, function (err, stats) {
    ...
    key: '__download',
    value: function __download(url, localPath, start, end, progressCb) {
      debug('download bytes ' + start + '-' + end + ' from ' + url + ' to ' + localPath);

      return new Promise(function (resolve, reject) {
        var writable = _fs2.default.createWriteStream(localPath);
